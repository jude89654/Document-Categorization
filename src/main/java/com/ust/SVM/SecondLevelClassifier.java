...
public class SecondLevelClassifier {
    ...
    final String UNIQUE_FILENAME = secondLevelFolderName +File.separator+"unique_words.txt";

    protected void buildTrainData(String class_filename, String training_folder) throws IOException {
        new File(secondLevelFolderName).mkdir();
    ...
    private void readTestClassNames(String test_class_filename) throws NumberFormatException, IOException {
        ...
        for (File file:
             new File(Main.TEMPORARY_FOLDER_PATH +File.separator+Main.SECOND_DEV_FOLDER_NAME).listFiles(new FileFilter() {
                 @Override
